...
Gauss Jordan
"""
function gauss_jordan(M::Matrix{Float64}, m::Vector{Float64})
    A = copy(M); b = copy(m)
    rows = 1:size(A)[1]
    for row in rows
...
function lu_decom(A::Matrix{Float64}, b::Vector{Float64}, returnlu=false)
    nn = size(A)
...
Jacobi
"""
function jacobi(A::Matrix{Float64}, b::Vector{Float64}, x=ones(Float64, size(A)[1], ), stop=1e-5)
...
Gauss Siedel
"""
function gauss_siedel(A::Matrix{Float64}, b::Vector{Float64}, x=ones(Float64, size(A)[1], ), stop=1e-10)
    oldx = zeros(Float64, size(A)[1], ) 
    while (oldx - x)' * (oldx - x) > stop
        if ((oldx - x)' * (oldx - x)) > 1e2 
            error("Did not converge, make sure coefficient matrix is diagonal heavy")
...
Conjugate Gradient
"""
function conjugate_gradient(A::Matrix{Float64}, b::Vector{Float64}, x=ones(Float64, size(A)[1], ), stop=1e-10)
    d = copy(b - A * x)
    r = copy(b - A * x)
    for i in 1:size(A)[1]
        Ad = A * d
